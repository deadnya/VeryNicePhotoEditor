...
class Spline {
    ...
    private fun drawDot(mutableBitmap: Bitmap, dotSize: Int, x: Int, y: Int, color: Int) {
    ...
    private fun fxaa(mutableBitmap: Bitmap, destPixels: IntArray, x: Int, y: Int) {
        ...
        val grayTL = getGrayscaleValue(
            mutableBitmap.getPixel(
                (x - 1).coerceAtLeast(0).coerceAtMost(width - 1),
                (y - 1).coerceAtLeast(0).coerceAtMost(height - 1)
            )
        )

        val grayTR = getGrayscaleValue(
            mutableBitmap.getPixel(
                (x + 1).coerceAtLeast(0).coerceAtMost(width - 1),
                (y - 1).coerceAtLeast(0).coerceAtMost(height - 1)
            )
        ...
        val dirReduce =
            ((grayTL + grayTR + grayBL + grayBR) * reduceMul * 0.25).coerceAtLeast(reduceMin)
        val inverseDirAdjust = 1.0 / (abs(initDirX).coerceAtMost(abs(initDirY)) + dirReduce)

        val dirX = (initDirX * inverseDirAdjust).coerceAtLeast(-spanMax).coerceAtMost(spanMax)
        val dirY = (initDirY * inverseDirAdjust).coerceAtLeast(-spanMax).coerceAtMost(spanMax)

        val pixel11 = mutableBitmap.getPixel(
            (x + (dirX * (1.0 / 3.0 - 0.5)).toInt()).coerceAtLeast(0).coerceAtMost(width - 1),
            (y + (dirY * (1.0 / 3.0 - 0.5)).toInt()).coerceAtLeast(0).coerceAtMost(height - 1)
        )

        val pixel12 = mutableBitmap.getPixel(
            (x + (dirX * (2.0 / 3.0 - 0.5)).toInt()).coerceAtLeast(0).coerceAtMost(width - 1),
            (y + (dirY * (2.0 / 3.0 - 0.5)).toInt()).coerceAtLeast(0).coerceAtMost(height - 1)
        )

        val pixel21 = mutableBitmap.getPixel(
            (x + (dirX * -0.5).toInt()).coerceAtLeast(0).coerceAtMost(width - 1),
            (y + (dirY * -0.5).toInt()).coerceAtLeast(0).coerceAtMost(height - 1)
        )
...
class MainDot(
    ...
    fun getPrevDot(): AnchorDot? {
        return prevDot
    }

    fun getNextDot(): AnchorDot? {
        return nextDot
    }
}
