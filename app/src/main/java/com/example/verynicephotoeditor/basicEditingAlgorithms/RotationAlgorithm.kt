...
class RotationAlgorithm {
    ...
    suspend fun rotateBitmap(source: Bitmap, degrees: Double): Bitmap =
        withContext(Dispatchers.Default) {
            ...
            for (xPrime in 0 until newWidth) {
                for (yPrime in 0 until newHeight) {
                    val x = (xPrime - newWidth / 2.0) * cos + (yPrime - newHeight / 2.0) * sin + px
                    val y = -(xPrime - newWidth / 2.0) * sin + (yPrime - newHeight / 2.0) * cos + py
                    ...
                    if (x >= 0 && x < width && y >= 0 && y < height) {
                        ...
                        rotatedBitmap.setPixel(xPrime, yPrime, Color.rgb(red, green, blue))
                    }
                }
            }
